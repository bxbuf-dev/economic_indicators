...
# Конфигурация для разных типов строений
PERMIT_SERIES = [
    {'fred_id': 'PERMIT1', 'category': '1 unit'},
    {'fred_id': 'PERMIT24', 'category': '2-4 units'},
    {'fred_id': 'PERMIT5', 'category': '5+ units'},
    {'fred_id': 'PERMIT', 'category': 'total'}  # Общий показатель
]

def main():
    """
    Коллектор данных Building Permits из FRED API.
    Загружает данные для всех типов строений в одну таблицу с категориями.
    """
    dao = None
    try:
        ...
        # Загружаем данные для каждого типа разрешений
        total_records = 0
        for series_config in PERMIT_SERIES:
            print(f"\nЗагрузка серии {series_config['fred_id']} ({series_config['category']})...")
            
            series_df = get_fred_series_history(
                series_id=series_config['fred_id'],
            ...
            print(f"Найдено {len(series_df)} записей для {series_config['category']}. Сохраняем в БД...")
