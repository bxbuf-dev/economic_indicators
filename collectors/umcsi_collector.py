...
INDICATOR_CONFIG = {
    ...
    'description': 'Consumer sentiment index based on surveys conducted by the University of Michigan. Base year 1966=100.'
}
...
def main():
    """
    Коллектор данных UMCSI из FRED API.
    """
    dao = None
    try:
        print("--- Запуск сборщика данных University of Michigan Consumer Sentiment Index ---")
        dao = IndicatorDAO()
        ...
        umcsi_df = get_fred_series_history(
        ...
        print(f"Найдено {len(umcsi_df)} записей для обработки. Сохраняем в БД (дубликаты будут проигнорированы)...")
        for _, row in umcsi_df.iterrows():
